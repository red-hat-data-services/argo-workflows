...
// NewMuxHandler returns an HTTP handler that multiplexes between gRPC and
// HTTP requests over the same port. It routes requests based on protocol version
// and Content-Type headers:
//   - HTTP/2 requests with Content-Type "application/grpc*" are routed to the gRPC handler
//   - All other requests are routed to the HTTP handler
