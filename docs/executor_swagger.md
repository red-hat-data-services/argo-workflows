...
| clusterName | string| `string` |  | | Deprecated: ClusterName is a legacy field that was always cleared by</br>the system and never used; it will be removed completely in 1.25.</br></br>The name in the go struct is changed to help clients detect</br>accidental use.</br></br>+optional |  |
...
| supplementalGroups | []int64 (formatted integer)| `[]int64` |  | | A list of groups applied to the first process run in each container, in addition</br>to the container's primary GID.  If unspecified, no groups will be added to</br>any container.</br>Note that this field cannot be set when spec.os.name is windows.</br>+optional |  |
