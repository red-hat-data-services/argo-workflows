...
func TestSidecarWithVolume(t *testing.T) {
	...
	assert.NoError(t, err)
	assert.Positive(t, len(pods.Items), "pod was not created successfully")
	pod := pods.Items[0]
...
func TestSuspendInputsResolution(t *testing.T) {
	...
	assert.Equal(t, 3, len(node.Inputs.Parameters[0].Enum))
...
func TestWFWithRetryAndWithParam(t *testing.T) {
	t.Run("IncludeScriptOutputInRetryAndWithParam", func(t *testing.T) {
		...
		pods, err := listPods(woc)
		assert.NoError(t, err)
		assert.Positive(t, len(pods.Items))
		if assert.Len(t, pods.Items, 3) {
...
`)
	...
	woc := newWorkflowOperationCtx(wf, controller)
	assert.Positive(t, len(woc.wf.Status.Nodes))
...
func TestRetryTransitionWorks(t *testing.T) {
	...
	for _, nodeName := range retryNodes {
		...
		assert.True(node.NodeFlag.Retried)
...
func TestOnExitTransitionWorks(t *testing.T) {
	...
	assert.False(node.NodeFlag.Retried)
